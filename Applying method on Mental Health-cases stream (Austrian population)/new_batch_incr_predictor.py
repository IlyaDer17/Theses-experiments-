...
class new_bath_incr_predictor:
    ...
    def ensambles_qialities(self, weights, n_iters=300, fraq_bootstrap=0.8) -> dict:
    ...
    def one_learner_quality(self, shuffle=True, n_iters=300, fraq_bootstrap=0.8):
        # creating qualities
        for i in range(self.number_intervals - 1):  # -1 because we can't test quality for last moment
            batches_moment_i = {inter: batch for inter, batch in self.train_batches.items() if inter[1] == i}
            test_data = self.train_batches[(i + 1, i + 1)]
    ...
    def plot_metrics(self):
        x = list(map(lambda x: f"batch #{x} ({self.num_observ_batches[x]})", np.arange(
            self.number_intervals - 1)))  # -1 because we cannot calculate quality for the last interval
        fig = go.Figure()
    ...
    def make_expirement(self, type_clust="static") -> list:
        ...
        if type_clust == "static":
            ...
            self.num_observ_batches = {inters[0]: batch.query("t_point == 't_0'").index.nunique() for inters, batch in
                                       batches.items() if
                                       inters[0] == inters[1]}
